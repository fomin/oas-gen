package jsm.java.rest.operations

import jsm.*
import jsm.java.*
import jsm.java.jackson.*
import jsm.java.spring.mvc.MessageConverterWriter
import java.util.*
        ...
        items.forEach { openApiSchema ->
            ...
            val operationMethods = javaOperations.map { javaOperation ->
                val requestBodyArgDeclaration = javaOperation.requestVariable?.let { requestVariable ->
                    "${requestVariable.type} ${toVariableName(getSimpleName(requestVariable.type))}"
                }
                val requestBodyInternalArgDeclaration = javaOperation.requestVariable?.let { requestVariable ->
                    "${requestVariable.type} bodyArg"
                }
                ...
                val (requestType, buildRequestExpression) = when {
                    javaOperation.requestVariable != null -> Pair(
                            javaOperation.requestVariable.type,
                            """|.contentType(MediaType.APPLICATION_JSON)
                               |.body(bodyArg, ${javaOperation.requestVariable.type}.class);
                               |""".trimMargin()
                    )
                    else -> Pair(
                            "java.lang.Void",
